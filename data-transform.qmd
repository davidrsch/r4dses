...
1.  Compare `air_time` con `arr_time - dep_time`.
    ¿Qué esperas ver?
    ¿Qué ves?
    ¿Qué necesitas hacer para arreglarlo?

2.  Compare `dep_time`, `sched_dep_time` y `dep_delay`.
    ¿Cómo esperarías que se relacionaran esos tres números?

3.  Haga una lluvia de ideas sobre tantas formas como sea posible para seleccionar `dep_time`, `dep_delay`, `arr_time` y `arr_delay` de `flights`.

4.  ¿Qué sucede si incluye el nombre de una variable varias veces en una llamada `select()`?

5.  ¿Qué hace la función `any_of()`?
