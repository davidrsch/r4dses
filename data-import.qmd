...
Por ejemplo, `meal_type` es una variable categórica con un conjunto conocido de valores posibles, que en R debería representarse como un factor:
...
Tenga en cuenta que los valores en la variable `meal_type` se han mantenido igual, pero el tipo de variable indicado debajo del nombre de la variable ha cambiado de carácter (`<chr>`) a factor (`<fct>`).
...
Llegaremos a la importación de datos varias veces en este libro: @sec-import-spreadsheets desde Excel y Googlesheets, @sec-import-databases le mostrará cómo cargar datos desde bases de datos, @sec-arrow desde archivos parquet, \@ sec-rectangling de JSON y @sec-scraping de sitios web.
