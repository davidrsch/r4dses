...
Alternativamente, es posible que solo desee agregar una sola etiqueta al gráfico, pero aún necesitará crear un data frame.
A menudo, desea que la etiqueta esté en la esquina del gráfico, por lo que es conveniente crear un nuevo data frame usando `summarize()` para calcular los valores máximos de x e y.
...
Alternativamente, podemos agregar la anotación sin crear un nuevo data frame, usando `annotate()`.
Esta función agrega un geom a un gráfico, pero no asigna variables de un data frame a una estética.
...
Otro enfoque para llamar la atención sobre una característica de la gráfica es usar un segmento geom con el argumento `arrow`.
...
Esto es útil para mapas o para publicar gráficas donde no puede compartir los números absolutos.
...
#|   se coloca a la izquierda, arriba, abajo y a la derecha del gráfico.
...
#|   horizontalmente en una fila. Los puntos en la leyenda son más grandes que los puntos
#|   en la gráfica
...
Los dos gráficos a continuación se ven similares, pero hay suficiente diferencia en los tonos de rojo y verde para que los puntos de la derecha puedan ser distinguidos incluso por personas con daltonismo rojo-verde.
...
No olvides las técnicas más simples.
...
```{r}
#| fig-alt: >
#|   Dos diagramas de dispersión de kilometraje en carretera versus tamaño de motor donde ambos colores
...
Tenga en cuenta que todas las escalas de color vienen en dos variedades: `scale_color_x()` y `scale_fill_x()` para la estética `color` y `fill` respectivamente (las escalas de color están disponibles en la ortografía del Reino Unido y EE. UU.).
...
Para acercar una región de la gráfica, generalmente es mejor usar `coord_cartesian()`.
Compara las dos gráficas siguientes:
...
También puede establecer los "límites" en escalas individuales.
Reducir los límites es básicamente equivalente a dividir los datos en subconjuntos.
Por lo general, es más útil si desea *expandir* los límites, por ejemplo, para hacer coincidir escalas en diferentes gráficos.
...
3.  Cambie la visualización de los términos presidenciales por:
    ...
    a.  Combinando las dos variantes mostradas arriba.
...
4.  Use `override.aes` para hacer que la leyenda en el siguiente gráfico sea más fácil de ver.

    ```{r}
    #| fig-format: "png"
    #| out-width: "50%"
    #| fig-alt: >
    #|   Diagrama de dispersión de precio versus quilate de diamantes. Los puntos son de colores.
    #|   por corte de los diamantes y son muy transparentes.
...
ggplot2 incluye ocho temas por defecto, como se muestra en @fig-themes.
...
Muchas personas se preguntan por qué el tema predeterminado tiene un fondo gris.
...
El fondo gris le da a la gráfica un color tipográfico similar al del texto, asegurando que los gráficos encajen con el flujo de un documento sin sobresalir con un fondo blanco brillante.
Finalmente, el fondo gris crea un campo continuo de color que asegura que la gráfica se perciba como una sola entidad visual.
...
ggplot(mpg, aes(x = displ, y = hwy, color = drv)) +
  geom_point() +
  labs(
    title = "El kilometraje en carretera disminuye a medida que aumenta el tamaño del motor",
...
También puede crear diseños de gráfica arbitrarios con patchwork.
...
#|   diagrama de dispersión de kilometraje en carretera versus kilometraje en ciudad, y el tercer diagrama es
#|   diagramas de caja de lado a lado de kilometraje en carretera versus tren de transmisión) colocados a continuación
#|   el uno al otro
...
A continuación, tenemos 5 gráficas.
