...
Ahora vamos a acercarnos un poco y hablar sobre la traducción de las funciones R que funcionan con columnas individuales, p. ¿Qué pasa cuando usas `mean(x)` en `summarize()`?
...
En SQL, la cláusula `GROUP BY` se usa exclusivamente para resúmenes, por lo que aquí puede ver que la agrupación se ha movido al argumento `PARTITION BY` a `OVER`.
...
Usamos esa traducción para enseñarle un poco de SQL; es importante aprender algo de SQL porque es *el* lenguaje más comúnmente usado para trabajar con datos y saberlo te facilitará comunicarte con otras personas de datos que no usan R.
