...
La frustración es natural cuando comienzas a programar en R porque es muy estricto con la puntuación, e incluso un carácter fuera de lugar hará que se queje.
...
Puedes usar R como calculadora:
...
Y la aritmética básica se aplica a cada elemento del vector:
...
El código es miserable para leer en un buen día, así que tómese un respiro y use espacios.

## Comentarios

R ignorará cualquier texto después de `#`.
...
Pero si describe el "qué" en sus comentarios y su código, deberá recordar actualizar el comentario y el código en conjunto con cuidado.
Si cambia el código y olvida actualizar el comentario, serán inconsistentes, lo que generará confusión cuando regrese a su código en el futuro.
...
Ups, cometiste un error!
El valor de `este_es_un_nombre_realmente_largo` debería ser 3.5, no 2.5.
Use otro método abreviado de teclado para ayudarlo a solucionarlo.
Escribe "este" y luego presiona Cmd/Ctrl + ↑.
Al hacerlo, aparecerá una lista de todos los comandos que hayas escrito que comiencen con esas letras.
...
Escriba los argumentos `1, 10`.

```{r}
seq(1, 10)
```

Escriba este código y observe que RStudio proporciona una asistencia similar con las comillas emparejadas:
