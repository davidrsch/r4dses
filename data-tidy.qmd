...
-   `names_to` nombres de la variable almacenada en la columna nombres, llamamos a esa variable `week`.
...
Como se muestra en @fig-pivot-variables, los valores en la columna que ya era una variable en el conjunto de datos original (`var`) deben repetirse, una vez por cada columna que se pivote.
...
La primera pieza, `sp`/`rel`/`ep`, describe el método utilizado para el diagnóstico, la segunda pieza, `m`/`f` es el `género` (codificado como una variable binaria en este conjunto de datos) , y la tercera pieza, `014`/`1524`/`2535`/`3544`/`4554`/`65` es el rango de `edad` (`014` representa 0-14, por ejemplo).
...
El siguiente ejemplo tiene dos filas que corresponden a la identificación "A" y al nombre "bp1":
