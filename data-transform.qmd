...
Dado que cada verbo es bastante simple, resolver problemas complejos generalmente requerirá combinar varios verbos, y lo haremos con las canalizaciones o pipes `|>`.
Hablaremos más sobre las canalizaciones en @the-pipe, pero en resumen, una canalización toma la cosa a su izquierda y la pasa a la función a su derecha para que `x |> f(y)` sea equivalente a `f(x, y)`, y `x |> f(y) |> g(z)` es equivalente a `g(f(x, y), z)`.
...
Tenga en cuenta que si desea encontrar la cantidad de duplicados o filas que no se duplicaron, es mejor que cambie `distinct()` por `count()`, lo que le dará la cantidad de observaciones por nivel único, y luego filtre según sea necesario.
...
`mutate()` crea nuevas columnas que se derivan de las columnas existentes; `select()`, `rename()` y `relocate()` cambian qué columnas están presentes, sus nombres o sus posiciones.
...
## Canalizaciones o pipe {#the-pipe}
...
Entonces, ¿por qué recomendamos la tubería base?
...
En segundo lugar, `|>` es un poco más simple que `%>%`: en el tiempo entre la invención de `%>%` en 2014 y la inclusión de `|>` en R 4.1.0 en 2021, ganamos una mejor comprensión de la tubería.
...
Volveremos a discutir los valores faltantes en detalle en @sec-missing-values, pero por ahora los eliminaremos usando `na.rm = TRUE`:
...
En concreto, compararemos la proporción de veces que un jugador acierta vs el número de veces que intenta poner la pelota en juego:
...
1.  La variación en nuestro agregado disminuye a medida que obtenemos más puntos de datos.
...
2.  Hay una correlación positiva entre la habilidad (`perf`) y las oportunidades de golpear la pelota (`n`) porque los equipos quieren dar a sus mejores bateadores la mayor cantidad de oportunidades para golpear la pelota.
...
Si selecciona ingenuamente `desc(performance)`, las personas con los mejores promedios de bateo son claramente afortunadas, no hábiles:
