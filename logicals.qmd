...
2.  Dado un vector de días como `x <- c("Lunes", "Sábado", "Miércoles")`, use una instrucción `ifelse()` para etiquetarlos como fines de semana o días de semana.

3.  Usa `ifelse()` para calcular el valor absoluto de un vector numérico llamado `x`.
