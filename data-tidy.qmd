...
1.  Usando palabras, describa cómo se organizan las variables y las observaciones en cada una de las tablas de muestra.

2.  Haz un bosquejo del proceso que usarías para calcular la `rate` para `table2` y `table3`.
...
#| echo: false
#| fig-cap: >
#|   Los nombres de columna de las columnas pivoteadas se convierten en una nueva columna. Los valores necesitan repetirse una vez por cada fila en el conjunto de datos original
#| fig-alt: >
#|   Un diagrama que muestra cómo `pivot_longer()` transforma un simple
...
Encontrar los nombres de las columnas es fácil: son solo los valores únicos de `name`.
...
Los ejemplos que usamos aquí son solo una selección de los de `vignette(pivot, package = "tidyr")`, por lo que si encuentra un problema con el que este capítulo no le ayuda, esa viñeta es un buen lugar para intentarlo a continuación.
