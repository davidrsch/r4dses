...
Cuando renderiza el documento, Quarto envía el archivo `.qmd` a **knitr**, [https://yihui.name/knitr](https://yihui.name/knitr/){.uri}, que ejecuta todos los fragmentos de código y crea un nuevo documento de markdown (`.md`) que incluye el código y su salida.
...
Puedes ver la lista completa en [https://yihui.name/knitr/options](https://yihui.name/knitr/options/){.uri}.
...
Para incrustar una imagen de un archivo externo, puede usar el menú Insertar en RStudio y seleccionar Figure / Image.
...
-   Controle el tamaño de salida con `out-width` y configúrelo en un porcentaje del ancho de línea.
...
Por ejemplo, si su `fig-width` predeterminado es 6 y `out-width` es 0,7, cuando establezca `out-width: "50%"`, deberá establecer `fig-width` en 4.3 (6 \* 0.5 / 0.7).
...
Una buena función para usar es `file.info()`: devuelve un montón de información sobre el archivo, incluida la última vez que se modificó.
Entonces puedes escribir:

```         
`r chunk`{r}
#| label: raw-data
#| cache.extra: file.info("un_archivo_muy_grande.csv")
...
De forma predeterminada, estas dependencias se colocan en un directorio `_files` junto a su documento.
Por ejemplo, si renderiza `report.qmd` a HTML:
...
También puede ejecutar expresiones R arbitrarias anteponiendo el valor del parámetro con `!r`.
