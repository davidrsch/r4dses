...
**Las clases de caracteres** están definidas por `[]` y le permiten hacer coincidir un conjunto de caracteres, p. `[abcd]` coincide con "a", "b", "c" o "d".
...
#| label: fig-x-names
#| fig-cap: >
#|   Una serie de tiempo que muestra la proporción de nombres de bebés que contienen un
#|   "x" minúscula.
#| fig-alt: >
...
Hay dos funciones que están estrechamente relacionadas con `str_detect()`, a saber, `str_subset()` que devuelve solo las cadenas que contienen una coincidencia y `str_which()` que devuelve los índices de las cadenas que tienen una coincidencia:
...
El siguiente paso en complejidad de `str_detect()` es `str_count()`: en lugar de un simple verdadero o falso, te dice cuántas coincidencias hay en cada cadena.
...
-   Dígale a la expresión regular que ignore el tamaño: `str_count(regex(name, ignore_case = TRUE), "[aeiou]")`. Hablaremos de más en @sec-flags.
...
Para hacer coincidir un `.` literal, necesita un **escape** que le indique a la expresión regular que coincida con los metacaracteres literalmente.
Al igual que las cadenas, las expresiones regulares usan la barra invertida para escapar.
Entonces, para hacer coincidir un `.`, necesita la expresión regular `\.`.
Desafortunadamente esto crea un problema.
...
En general, mire los caracteres de puntuación con suspicacia; si su expresión regular no coincide con lo que cree que debería, compruebe si ha utilizado alguno de estos caracteres.

### Anclajes

De forma predeterminada, las expresiones regulares coincidirán con cualquier parte de una cadena.
Si desea hacer coincidir al principio del final, necesita **anclar** la expresión regular usando `^` para que coincida con el comienzo de la cadena o `$` para que coincida con el final de la cadena:
...
-   `-` define un rango, p. `[a-z]` coincide con cualquier letra minúscula y `[0-9]` coincide con cualquier número.
...
Hay otros tres pares particularmente útiles[^regexps-6]:

[^regexps-6]: Recuerde, para crear una expresión regular que contenga `\d` o `\s`, deberá escapar del `\` para la cadena, por lo que escribirá `"\\d"` o `"\\s "`.
...
El siguiente código muestra cómo funciona esto para algunos ejemplos simples:
...
¿Coincide con la cadena completa a o con la cadena b completa, o con una cadena que comienza con a o con una cadena que comienza con "b"?
...
Pero `str_match()` devuelve una matriz, por lo que no es particularmente fácil trabajar con [^regexps-7]:

[^regexps-7]: ¡Sobre todo porque nunca hablamos de matrices en este libro!
...
Esto le permite usar comentarios y espacios en blanco para hacer que las expresiones regulares complejas sean más comprensibles[^regexps-8], como en el siguiente ejemplo:
...
-   `matches(pattern)` seleccionará todas las variables cuyo nombre coincida con el patrón proporcionado.
    Es una función "tidyselect" que puede usar en cualquier lugar en cualquier función tidyverse que seleccione variables (p. `select()`, `rename_with()` y `across()`).
