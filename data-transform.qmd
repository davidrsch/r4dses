...
La visualización es una herramienta importante para generar información, pero es raro que obtenga los datos exactamente en la forma correcta para visualizarla.
...
Puede usar `desc()` para reordenar el data frame basandose en una columna, en orden descendente.
Por ejemplo, este código muestra los vuelos más retrasados primero:
...
Un argumento particularmente útil es `"used"`, que le permite mantener solo las entradas y salidas de sus cálculos.
...
3.  ¿Qué sucede si especifica el nombre de una variable varias veces en una llamada `select()`?
...
Esto es similar a calcular el retraso máximo con `summarize()`, pero obtiene la fila correspondiente completa en lugar de la estadística de resumen única:
