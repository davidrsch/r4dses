...
El editor de secuencias de comandos es un excelente lugar para colocar el código que le interesa.
Siga experimentando en la consola, pero una vez que haya escrito un código que funcione y haga lo que desea, colóquelo en el editor de secuencias de comandos.
...
¡Es muy antisocial cambiar la configuración en la computadora de otra persona!
...
Como usuario principiante de R, está bien considerar su entorno (es decir, los objetos enumerados en el panel de entorno) como su análisis.
Sin embargo, a la larga, estará mucho mejor si se asegura de que sus scripts R sean la fuente de la verdad.
...
Puede hacer esto ejecutando `usethis::use_blank_slate()`[^workflow-scripts-1] o imitando las opciones que se muestran en @fig-blank-slate. Esto le causará un poco de dolor a corto plazo, porque ahora, cuando reinicie RStudio, ya no recordará el código que ejecutó la última vez.
Pero este dolor a corto plazo le ahorra una agonía a largo plazo porque lo obliga a capturar todas las interacciones importantes en su código.
No hay nada peor que descubrir tres meses después del hecho de que solo almacenó los resultados de un cálculo importante en su espacio de trabajo, no el cálculo en sí mismo en su código.
...
#| fig-alt: >
#|   Ventana de preferencias de RStudio donde aparece la opción Restaurar .RData en el espacio de trabajo
...
1.  Presione Cmd/Ctrl + Shift + F10 para reiniciar R.
...
Pero llevas nueve capítulos en este libro y ya no eres un principiante de rango.
...
#| echo: false
#| fig-cap: > 
#|   Cree un nuevo proyecto siguiendo estos tres pasos.
#| fig-subcap: 
#|   - Primero haga clic en Nuevo directorio.
#|   - Luego haga clic en Nuevo proyecto.
#|   - Finalmente, complete el nombre del directorio (proyecto), elija un buen
#|   subdirectorio para su inicio y haga clic en Crear proyecto.
#| fig-alt: >
...
A continuación, ejecute el script completo que guardará un archivo PDF y CSV en el directorio de su proyecto.
...
De la forma específica de su sistema operativo favorito, busque `diamonds.pdf` en su computadora y encontrará el PDF (no es de extrañar), pero *también el script que lo creó* (`diamonds.R`).
...
Cuando Hadley escribió `diamonds.R` arriba, era un atajo para `/Users/hadley/Documents/r4ds/r4ds/diamonds.R`.
Pero lo que es más importante, si Mine ejecutara este código en su computadora, apuntaría a `/Users/Mine/Documents/r4ds/r4ds/diamonds.R`.
Esta es la razón por la que las rutas relativas son importantes: funcionarán independientemente de dónde termine el proyecto.
...
Mac y Linux usan barras inclinadas (por ejemplo, `plots/diamonds.pdf`) y Windows usa barras invertidas (por ejemplo, `plots\diamonds.pdf`).
...
En resumen, los scripts y los proyectos le brindan un flujo de trabajo sólido que le será útil en el futuro:
...
A continuación, aprenderá cómo obtener ayuda y cómo hacer buenas preguntas de codificación.
