...
Sin embargo, conocer el paquete puede ayudarlo a encontrar ayuda y funciones relacionadas, por lo que cuando necesitemos ser precisos sobre de qué paquete proviene una función, usaremos la misma sintaxis que R: `nombrepaquete::nombrefuncion()`.
...
Los datos provienen de la [Oficina de Estadísticas de Transporte](https://www.transtats.bts.gov/DL_SelectFields.aspx?gnoyr_VQ=FGJ&QO_fu146_anzr=b0-gvzr) de EE.
UU.
...
Estos son importantes porque las operaciones que puede realizar en una columna dependen mucho de su "tipo".
...
Aprenderemos más sobre lo que sucede aquí y por qué en @sec-order-operations-boolean.
...
Y si queremos que estén disponibles en un data frame para uso futuro, debemos pensar detenidamente si queremos que el resultado se asigne de nuevo a `flights`, sobrescribiendo el data frame original con muchas más variables, o a un nuevo objeto.
...
-   Seleccionar todas las columnas excepto aquellas entre year y day (incluyendolas)
    ...
    Históricamente, esta operación se realizaba con `-` en lugar de `!`, por lo que es probable que la veas en la naturaleza.
    Estos dos operadores tienen el mismo propósito pero con sutiles diferencias de comportamiento.
    Recomendamos usar `!` porque se lee como "no" y combina bien con `&` y `|`.
...
6.  Supongamos que tenemos el siguiente data frame diminuto:
    ...
    f.  Escriba cómo cree que se verá la salida, luego verifique si estuvo en lo correcto y describa qué hace la canalización.
        ¿En qué se diferencian las salidas de las dos *canalizaciones*?
