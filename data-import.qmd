...
Por defecto, `read_csv()` solo reconoce cadenas vacías (`""`) en este conjunto de datos como `NA`s, queremos que también reconozca la cadena de caracteres `"N/A"`.
...
Antes de analizar estos datos, probablemente querrá arreglar las columnas `age` y `id`.
...
Hay un par de otros argumentos importantes que debemos mencionar, y serán más fáciles de demostrar si primero le mostramos un truco útil: `read_csv()` puede leer cadenas de texto que ha creado y formateado como un archivo CSV:
...
Una de las causas más comunes de esto es un valor faltante, registrado usando algo diferente al `NA` que espera readr.
...
Puede hacerlo con el argumento `col_types`, que toma una lista con nombre donde los nombres coinciden con los nombres de las columnas en el archivo CSV:
...
-   `col_integer()` lee números enteros. Rara vez distinguimos números enteros y dobles en este libro porque son funcionalmente equivalentes, pero la lectura explícita de números enteros ocasionalmente puede ser útil porque ocupan la mitad de la memoria de los dobles.
-   `col_character()` lee cadenas de caracteres. Esto a veces es útil para especificar explícitamente cuando tiene una columna que es un identificador numérico, es decir, una larga serie de dígitos que identifica algún objeto, pero no tiene sentido (por ejemplo) dividirlo por la mitad, por ejemplo, un número de teléfono , número de seguro social, número de tarjeta de crédito, etc.
...
-   `col_skip()` salta una columna por lo que no se incluye en el resultado, que puede ser útil para acelerar la lectura de datos si tiene un archivo CSV grande y solo desea usar algunas de las columnas.

También es posible anular la columna predeterminada cambiando de `list()` a `cols()` y especificando `.default`:
...
Una vez más, el código anterior funcionará si tiene los archivos CSV en una carpeta `data` en su proyecto.
Puede descargar estos archivos desde <https://pos.it/r4ds-01-sales>, <https://pos.it/r4ds-02-sales> y \<https://pos.it/r4ds- 03-ventas\> o puedes leerlos directamente con:
...
El argumento `id` agrega una nueva columna llamada `file` al data frame resultante que identifica el archivo del que provienen los datos.
...
Los argumentos más importantes para estas funciones son `x` (el data frame a guardar) y `file` (la ubicación para guardarlo).
...
Tenga en cuenta que la información del tipo de variable que acaba de configurar se pierde cuando guarda en CSV porque está comenzando de nuevo con la lectura de un archivo de texto sin formato:
...
1.  `write_rds()` y `read_rds()` son envoltorios uniformes alrededor de las funciones base `readRDS()` y `saveRDS()`.
    ...
    Esto significa que cuando vuelve a cargar el objeto, está cargando el *exactamente el mismo* objeto R que almacenó.
