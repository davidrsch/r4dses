...
Todas estas funciones tienen una sintaxis similar al igual que otras funciones que hemos introducido anteriormente para leer otros tipos de archivos, p. `read_csv()`, `read_table()`, etc.
...
Otro desafío es cuando tiene una columna en su hoja de cálculo de Excel que tiene una combinación de estos tipos, p. algunas celdas son numéricas, otras de texto, otras de fechas.
...
1.  En un archivo de Excel, cree el siguiente conjunto de datos y guárdelo como `survey.xlsx`.
    ...
    Sugerencia: deberá convertir "none" a 0.
...
Google Sheets es otro programa de hoja de cálculo ampliamente utilizado incluido.
...
El primer argumento de `read_sheet()` es la URL del archivo a leer, y devuelve un tibble:
...
Sin embargo, si desea especificar una cuenta de Google específica, el alcance de la autenticación, etc., puede hacerlo con `gs4_auth()`, p. `gs4_auth(email = "mine@example.com")`, que forzará el uso de un token asociado con un correo electrónico específico.
