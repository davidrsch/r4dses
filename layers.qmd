...
Puede colorear un gráfico de barras utilizando la estética `color` o, más útil, `fill`:
...
#| fig-height: 2
#| fig-alt: >
#|   Dos gráficos de barras de corte de diamantes. En la primera gráfica, las barras tienen color
#|   en los bordes. En la segunda gráfica, están llenos de colores. Las alturas de 
#|   las barras corresponden al número de diamantes en cada categoría de corte.
...
Tenga en cuenta lo que sucede si asigna la estética de relleno a otra variable, como `clarity`: las barras se apilan automáticamente.
Cada rectángulo de color representa una combinación de `cut` y `clarity`.

```{r}
#| fig-alt: >
#|   Gráfico de barras segmentadas de corte de diamantes, donde cada barra se rellena con
#|   colores para los niveles de claridad. Las alturas de las barras corresponden al
#|   número de diamantes en cada categoría de corte y las alturas de los segmentos
#|   coloreados son proporcionales al número de diamantes con un nivel de claridad
#|   dado dentro de un nivel de corte.
...
-   `position = "identity"` colocará cada objeto exactamente donde cae en el contexto del gráfico.
    ...
    #| fig-height: 2
    #| fig-alt: >
    #|   Dos gráficos de barras segmentadas de corte de diamantes, donde se rellena cada barra
    #|   con colores para los niveles de claridad. Las alturas de las barras corresponden
    #|   al número de diamantes en cada categoría de corte, y las alturas de los
    #|   los segmentos coloreados son proporcionales al número de diamantes con un
    #|   nivel de claridad dado dentro de un nivel de corte dado. Sin embargo los segmentos
    #|   se superponen. En el primer gráfico, los segmentos se rellenan con colores
    #|   transparentes, en el segundo gráfico los segmentos solo están delineados con colores.
...
-   `position = "fill"` funciona como apilar, pero hace que cada conjunto de barras apiladas tenga la misma altura.
    ...
    #| fig-alt: >
    #|   Gráfico de barras segmentadas de corte de diamantes, donde cada barra se rellena con
    #|   colores para los niveles de claridad. La altura de cada barra es 1 y las alturas
    #|   de los segmentos coloreados son proporcionales a la proporción de diamantes
    #|   con un nivel de claridad dado dentro de un nivel de corte dado.
...
-   `position = "dodge"` coloca objetos superpuestos directamente *al lado* uno del otro.
    ...
    ```{r}
    #| fig-alt: >
    #|   Gráfico de barras multiples de corte de diamantes. Las barras multiples se agrupan por niveles
    #|   de corte (regular, bueno, muy bueno, premium e ideal). En cada grupo hay
    #|   ocho barras, una para cada nivel de claridad, y llenas de un
    #|   color diferente para cada nivel. Las alturas de estas barras representan el
    #|   número de diamantes con un determinado nivel de corte y claridad.
...
1.  ¿Cuál es el problema con esta gráfica?
    ¿Cómo podrías mejorarlo?

    ```{r}
    #| fig-alt: >
    #|   Diagrama de dispersión de eficiencia de combustible en carretera versus eficiencia de combustible en ciudad
    #|   de automóviles que muestra una asociación positiva. El número de puntos
    #|   visible en este gráfico es menor que el número de puntos en el conjunto de datos.

    ggplot(mpg, aes(x = cty, y = hwy)) + 
      geom_point()
    ```

2.  ¿Qué parámetros de `geom_jitter()` controlan la cantidad de fluctuación?

3.  Compare y contraste `geom_jitter()` con `geom_count()`.

4.  ¿Cuál es el ajuste de posición predeterminado para `geom_boxplot()`?
...
-   `coord_polar()` utiliza coordenadas polares.
    ...
    #| fig-asp: 1
    #| fig-alt: >
    #|   Hay dos gráficas. A la izquierda hay un gráfico de barras de corte de diamantes,
...
3.  ¿Qué te dice el siguiente diagrama sobre la relación entre mpg en ciudad y en carretera?
    ...
    #| fig-alt: >
    #|   Diagrama de dispersión de la eficiencia de combustible en carretera frente al tamaño del motor de los automóviles que
    #|   muestra una asociación negativa. La gráfica también tiene una línea recta que
    #|   sigue la tendencia de la relación entre las variables pero no
    #|   atraviesa la nube de puntos, está debajo de ella.
...
Podemos ampliar la plantilla gráfica que aprendió en @sec-graphing-template agregando ajustes de posición, estadísticas, sistemas de coordenadas y facetas:
...
La gramática de los gráficos se basa en la idea de que puede describir de manera única *cualquier* gráfico como una combinación de un conjunto de datos, una geom, un conjunto de asignaciones, una estadística, un ajuste de posición, un sistema de coordenadas y un esquema de facetas.
...
#| echo: false
#| fig-alt: >
#|   Una figura que demuestra los pasos para pasar de los datos sin procesar a la tabla de conteo
#|   donde cada fila representa un nivel de corte y una columna de conteo muestra cuántos
#|   diamantes están en ese nivel de corte. 
